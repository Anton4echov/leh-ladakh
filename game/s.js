window.addEventListener('load', function() {
        ...
        constructor(game) {
            ...
            window.addEventListener('touchmove', e => {
                ...
                if (Math.abs(deltaY) > 0) {
                    ...
                    this.game.touchVelocityY = Math.min(this.maxSpeed, Math.max(-this.maxSpeed, this.game.touchVelocityY + (direction * this.acceleration)));
                }
                if (Math.abs(deltaX) > 0) {
                    const direction = deltaX > 0 ? 1 : -1;
                    this.game.touchVelocityX = Math.min(this.maxSpeed, Math.max(-this.maxSpeed, this.game.touchVelocityX + (direction * this.acceleration)));
                }
            ...
            window.addEventListener('deviceorientation', e => {
                if (e.gamma !== null && e.beta !== null) {
                    const tiltX = e.gamma; 
                    const tiltY = e.beta;
                    this.game.tiltX = tiltX * 0.1; 
                    this.game.tiltY = tiltY * 0.05; 
                }
            });
        }
    ...
    class Player {
        ...
        update(deltaTime) {
            ...
            // === Tilt controls ===
            if (!this.game.keys.length && !this.game.isTouching) {
                this.speedX = this.game.tiltX || 0;
                this.speedY = this.game.tiltY || 0;
            }
